...
io.on("connection", async (socket) => {
  ...
  socket.on("start", (id) => {
    ...
    while (i < searching.length) {
      ...
      if (peer.id !== id) {
        ...
        io.of("/")
          .to(roomName)
          .emit("chatStart", "Jūs tikāt savienots ar nejaušu svešinieku.");
      ...
      socket.emit("searching", "Meklējam nejaušu svešinieku...");
  ...
  socket.on("stop", () => {
    ...
    peer.emit("strangerDisconnected", "Svešinieks pameta čatu.");

    // send message to current user
    socket.emit("endChat", "Jūs pametāt čatu.");
  ...
  socket.on("disconnecting", async () => {
    ...
    if (roomName) {
      // send message to room
      io.of("/").to(roomName).emit("goodBye", "Svešinieks pameta čatu.");
